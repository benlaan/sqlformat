...
namespace Laan.Sql.Parser.Test
{
    ...
    public class TestCastFunction : TestExpressionParser
    {
        ...
        [Test]
        public void CanCastDoublePrecision()
        {
            // setup
            var tokenizer = NewTokenizer("CAST(@Age AS DOUBLE PRECISION)");
            tokenizer.ReadNextToken();
            ...
            Assert.AreEqual("CAST", function.Name);
            Assert.AreEqual("@Age", function.Arguments.First().Value);
            Assert.AreEqual("DOUBLE PRECISION", function.OutputType.ToString());
            Assert.AreEqual("CAST(@Age AS DOUBLE PRECISION)", function.Value);
        }
